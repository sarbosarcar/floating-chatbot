...
Provide a concise answer to the user query based on the context.
"""

# Function to generate a chatbot response
def prompt_answer(query, context):
    # Construct the prompt
    prompt = PROMPT_TEMPLATE.format(context=context, query=query.strip().lower())
    messages = [
        {
            "role": "system",
    ...
    try:
        ...
        response = client.text_generation(prompt, temperature=0.2).strip()
